...
export default function MyPetsPage() {
  ...
  const loadMyPets = () => {
    ...
    const adoptedPets = pets.filter((p) => p.adoptedBy === user?.id);

    if (user?.role === "parent") {
      // Pets listed by the parent
      const listedPets = pets.filter((p) => p.parentId === user?.id);

      // Merge without duplicates
      const combined = [...adoptedPets];

      listedPets.forEach((pet) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 flex justify-between items-center">
        ...
        {/* ADD BUTTON: Redirects to My Adoptables page where the add logic exists */}
        {user?.role === "parent" && (
          <Button onClick={() => navigate("/adoptables")}>
