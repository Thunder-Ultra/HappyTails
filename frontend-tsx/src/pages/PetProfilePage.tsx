...
// Keys allowed to mutate in deleteEntry()
type HealthArrayKeys = keyof Pick<
  PetHealth,
  "vaccinations" | "medicalHistory" | "grooming" | "weightLogs"
>;
...
export default function PetProfilePage() {
  ...
  const [health, setHealth] = useState<PetHealth>({
    vaccinations: [],
    medicalHistory: [],
    grooming: [],
    weightLogs: [],
  ...
  useEffect(() => {
    ...
    const found = pets.find((p: Pet) => p.id === id);
  ...
  --------------------------------------------- */
  const saveHealthData = (updated: PetHealth) => {
    const allHealth: Record<string, PetHealth> = JSON.parse(
  ...
  --------------------------------------------- */
  const deleteEntry = (key: HealthArrayKeys, index: number) => {
    saveHealthData({
      ...health,
        ...
        <CardContent className="space-y-4">
          {health.vaccinations.length === 0 ? (
            ...
            <ul className="space-y-2">
              {health.vaccinations.map((item: string, index: number) => (
                <li
                  key={index}
      ...
      <Card className="mb-8">
        ...
        <CardContent className="space-y-4">
          {health.medicalHistory.length === 0 ? (
            ...
            <ul className="space-y-2">
              {health.medicalHistory.map((item: string, index: number) => (
                <li
                  key={index}
                  className="flex justify-between items-center bg-gray-50 p-2 rounded-md"
                >
      ...
      <Card className="mb-8">
        ...
        <CardContent className="space-y-4">
          {health.grooming.length === 0 ? (
            ...
            <ul className="space-y-2">
              {health.grooming.map((item: string, index: number) => (
                <li
                  key={index}
                  className="flex justify-between items-center bg-gray-50 p-2 rounded-md"
                >
      ...
      <Card className="mb-10">
        ...
        <CardContent className="space-y-4">
          {health.weightLogs.length === 0 ? (
            ...
            <ul className="space-y-2">
              {health.weightLogs.map((item: string, index: number) => (
                <li
                  key={index}
                  className="flex justify-between items-center bg-gray-50 p-2 rounded-md"
                >
