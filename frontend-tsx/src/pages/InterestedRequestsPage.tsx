...
export default function InterestedRequestsPage() {
  ...
  const [viewingRequest, setViewingRequest] = useState<ViewingRequestState | null>(null);
  ...
  const loadRequests = () => {
    const allRequests: AdoptionRequest[] = JSON.parse(
    ...
    if (index !== -1) {
      ...
      if (action === "accepted") {
        ...
        if (petIndex !== -1) {
          ...
          (pets[petIndex] as any).adoptedBy = viewingRequest.request.adopterId;
  ...
  // Helper functions
  // -----------------------------
  const getPetName = (id: string): string => {
    const pets: Pet[] = JSON.parse(localStorage.getItem("pets") || "[]");
    return pets.find((p) => p.id === id)?.name || "Unknown Pet";
  };

  const getAdopterName = (id: string): string => {
    const users: User[] = JSON.parse(localStorage.getItem("users") || "[]");
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {pendingRequests.length > 0 && (
        <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {pendingRequests.map((req: AdoptionRequest) => {
              const pets: Pet[] = JSON.parse(localStorage.getItem("pets") || "[]");
      ...
      {acceptedRequests.length > 0 && (
        <div className="mt-12">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {acceptedRequests.map((req: AdoptionRequest) => (
              <Card key={req.id}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div>
      ...
      {rejectedRequests.length > 0 && (
        <div className="mt-12">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {rejectedRequests.map((req: AdoptionRequest) => (
